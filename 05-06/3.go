...
func queryCaiYun(word string, wg *sync.WaitGroup) {
	...
	buffer.Write([]byte("彩云：\n"))
	buffer.Write([]byte(word + " UK:" + dictResponse.Dictionary.Prons.En + " US:" + dictResponse.Dictionary.Prons.EnUs))
...
func queryVolcano(word string, wg *sync.WaitGroup) {
	defer wg.Done()
